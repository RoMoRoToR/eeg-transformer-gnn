...
# Настройки ICA
ICA_NCOMP = 19  # Количество компонентов ICA
DO_ICA = True  # Применять ли ICA

# Параметры сегментации (скользящее окно)
WINDOW_SEC = 10.0  # Длительность окна в секундах (увеличено для получения более стабильных оценок)
...
def build_edge_index(num_channels=19, connectivity_matrix=None, threshold=0.5):
    ...
    Если передана connectivity_matrix (например, корреляционная матрица), то для каждой пары каналов,
    у которых значение превышает порог, создаётся ребро.
    Если connectivity_matrix отсутствует, используется фиксированная (например, кольцевая) топология.
    """
...
def process_one_subject(set_fname, l_freq=1., h_freq=40., do_ica=False, ica_ncomp=15):
    ...
    и опционально выполняет ICA для удаления артефактов.
...
def extract_channel_features(seg, sfreq):
    ...
    Входные параметры:
      seg: массив размера (C, T), где C — число каналов, T — число отсчетов.
      sfreq: частота дискретизации.
    Вычисляем:
      - Среднее значение (mean)
      - Стандартное отклонение (std)
      - Мощность в диапазонах: delta (1–4 Гц), theta (4–8 Гц), alpha (8–12 Гц),
        beta (12–30 Гц), gamma (30–45 Гц)
    Признаки масштабируются (умножением на 1e5) для удобства обучения.
    Опционально можно добавить z-score нормализацию (закомментирована строка).
    Возвращает массив (C, 7).
    """
    ...
    # Опционально: z-score нормализация признаков (раскомментируйте при необходимости)
...
def cumulative_graph_from_raw(raw, win_sec=5.0, step_sec=2.0, label=0, threshold=0.5):
    ...
    Для каждого окна:
      - Вычисляются признаки для каждого канала (размерность 7).
      - Рассчитывается матрица функциональной связности (например, корреляций).
      - Строится локальный граф с использованием порога (threshold).
    Затем:
      - Все узлы из всех окон объединяются в один граф.
      - Индексы ребер корректируются с помощью накопительного смещения.
      - Добавляются временные ребра, соединяющие узлы, соответствующие одному и тому же каналу,
        из соседних окон, для учета динамики сигнала.
    Возвращает объект Data (PyTorch Geometric) с признаками x, ребрами edge_index и меткой y.
    """
    sfreq = raw.info['sfreq']
    data_arr = raw.get_data()  # размер (C, N)
    ...
    node_features_list = []
    edge_indices_list = []
    window_indices = []  # для хранения смещений узлов каждого окна
    cumulative_offset = 0
    window_count = 0

    # Разбиение на окна (скользящее окно)
    while (window_count * step_samps + win_samps) <= n_times:
        start = window_count * step_samps
        seg = data_arr[:, start:start + win_samps]  # (C, T)

        # Извлечение признаков (размер (C, 7))
        features = extract_channel_features(seg, sfreq=sfreq)
        node_features_list.append(torch.tensor(features, dtype=torch.float))

        # Вычисление функциональной связности (например, корреляции)
        connectivity = np.corrcoef(seg)
        local_edge_index = build_edge_index(num_channels=n_channels,
                                            connectivity_matrix=connectivity,
                                            threshold=threshold)
        # Корректировка индексов для текущего окна
    ...
    # Объединяем признаки всех окон: итоговый тензор размера (num_windows * n_channels, 7)
    x = torch.cat(node_features_list, dim=0)

    # Объединяем локальные ребра всех окон
    ...
    # Добавляем временные ребра: для каждого канала соединяем узлы из соседних окон
    temporal_edges = []
    ...
    # Объединяем локальные и временные ребра
...
def main():
    ...
    for idx, row in participants_df.iterrows():
        ...
        try:
            ...
            # который учитывает локальные функциональные связи и временную динамику
...
if __name__ == "__main__":
    DO_ICA = False  # При необходимости измените настройки ICA
    main()
