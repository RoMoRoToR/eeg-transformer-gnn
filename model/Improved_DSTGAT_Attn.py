...
if __name__ == "__main__":
    ...
    torch.save(model.state_dict(), "improved_dstgcn_model_final_no_filter.pth")
...
def evaluate_and_visualize(model, dataloader, device, edge_index):
    model.eval()
    all_labels = []
    all_preds = []
    all_probs = []
    ...
    with torch.no_grad():
        for clips, labels in dataloader:
            ...
            latencies.extend([latency / clips.size(0)] * clips.size(0))  # на каждый сэмпл

            probs = torch.softmax(outputs, dim=1)[:, 1].cpu().numpy()  # вероятность класса 1
    ...
    # Latency vs Predicted Probability
    plt.figure(figsize=(6, 4))
    plt.scatter(all_probs, latencies, alpha=0.7)
...
# Вызов:
evaluate_and_visualize(model, test_loader, device, edge_index)
