...
###########################################
# Улучшенная модель DSTGAT_Attn с residual connections, SE-блоком и дополнительной оптимизацией
###########################################
...
if __name__ == "__main__":
    ...
    # Использование оптимизатора AdamW и добавление градиентного клиппинга
    optimizer = optim.AdamW(model.parameters(), lr=learning_rate, weight_decay=1e-4)
    scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=num_epochs)


    # Функция потерь с label smoothing (если требуется, здесь установлено значение 0.1)
    ...
    for epoch in range(num_epochs):
        ...
        scheduler.step()
        print(
            f"Эпоха {epoch + 1}/{num_epochs}: Обучение - Потери: {train_loss:.4f}, Точность: {train_acc:.4f} | Валидация - Потери: {val_loss:.4f}, Точность: {val_acc:.4f}")
