...
###########################################
# Training with subject-wise split
###########################################
def train_and_evaluate(root_dir, device, num_epochs=50):
    ...
    for subj in subjects:
        ...
        for f in sorted(os.listdir(subj_dir)):
            ...
            lab = int(np.load(p)['label'])
            if subj in train_subj:
                train_paths.append(p); train_labels.append(lab)
            else:
                val_paths.append(p); val_labels.append(lab)
    # Datasets
    train_ds = EEGDataset(train_paths, train_labels, augment=True)
    val_ds   = EEGDataset(val_paths,   val_labels,   augment=False)
    # sampler
    counts = np.bincount(train_labels); weights=1.0/counts
    sampler = WeightedRandomSampler([weights[l] for l in train_labels], len(train_paths), replacement=True)
    ...
    scheduler = OneCycleLR(optimizer, max_lr=5e-3, total_steps=steps, pct_start=0.2, div_factor=10, final_div_factor=10)
    best,wait=0,0
    ...
    for ep in range(1,num_epochs+1):
        ...
        for x,y in train_loader:
            ...
            preds=model(x,edge_index,bi); loss=criterion(preds,y)
            optimizer.zero_grad(); loss.backward()
            nn.utils.clip_grad_norm_(model.parameters(),1.0)
            optimizer.step(); scheduler.step()
            tl+=loss.item()*B; tc+=(preds.argmax(1)==y).sum().item(); tt+=B
        ...
        with torch.no_grad():
            for x,y in val_loader:
                ...
                bi=torch.arange(B,device=device).unsqueeze(1).repeat(1,C).view(-1)
                preds=model(x,edge_index,bi); loss=criterion(preds,y)
                vl+=loss.item()*B; vc+=(preds.argmax(1)==y).sum().item(); vt+=B
...
if __name__ == "__main__":
    ...
    train_and_evaluate("/Users/taniyashuba/PycharmProjects/eeg-transformer-gnn/data/preproc_clips", device)
