...
class EEGDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        clip = data['clip']  # Форма: [n_channels, n_samples]
        label = data['label']
...
if __name__ == "__main__":
    ...
    labels_list = []
    latencies = []
    probs_list = []  # Для хранения вероятностей (если нужна ROC-кривая)

    # Обработка каждого клипа по отдельности (имитация потоковых данных)
    for i in range(len(dataset)):
        clip, label = dataset[i]
        # Добавляем размерность батча: [1, n_channels, n_samples]
        ...
        # Для CrossEntropyLoss выход модели имеет размер [batch, num_classes]
        # Применяем torch.max для выбора класса
        _, pred = torch.max(output, dim=1)
        preds_list.append(pred.item())
        labels_list.append(label.item())
        probs_list.append(F.softmax(output, dim=1)[0,1].item())  # вероятность класса 1

        print(f"Клип {i+1}/{len(dataset)}: предсказано {pred.item()}, истинное {label.item()}, время обработки: {latency:.4f} сек.")
    ...
    # Вычисление Confusion Matrix и вывод отчёта по классификации
    ...
    # 4. ROC-кривая (если требуются дополнительные метрики)
    ...
    # 6. Зависимость задержки от вероятности предсказания (scatter plot)
    plt.figure()
    plt.scatter(probs_list, latencies, alpha=0.6, color='purple')
